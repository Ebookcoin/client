...
angular.module('ebookcoinApp').controller('multisignatureModalController', ["$scope", "$http", "multisignatureModal", "viewFactory", "userService", 'gettextCatalog', function ($scope, $http, multisignatureModal, viewFactory, userService, gettextCatalog) {
    ...
    $scope.addMember = function (contact) {
        ...
        if ($scope.contact.trim() == '') {
            ...
            if (buffer.length == 32) {
                ...
                var address = ebookcoin.crypto.getAddress($scope.contact);
