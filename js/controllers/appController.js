...
angular.module('ebookcoinApp').controller('appController', ['dappsService', '$scope', '$rootScope', '$http', "userService", "$interval", "$timeout", 'viewFactory', '$state', 'blockService', 'sendTransactionModal', 'registrationDelegateModal', 'userSettingsModal', 'serverSocket', 'delegateService', '$window', 'forgingModal', 'errorModal', 'contactsService', 'addContactModal', 'userInfo', 'transactionsService', 'secondPassphraseModal', 'focusFactory', 'gettextCatalog', function (dappsService, $rootScope, $scope, $http, userService, $interval, $timeout, viewFactory, $state, blockService, sendTransactionModal, registrationDelegateModal, userSettingsModal, serverSocket, delegateService, $window, forgingModal, errorModal, contactsService, addContactModal, userInfo, transactionsService, secondPassphraseModal, focusFactory, gettextCatalog) {
    ...
    $scope.getPriceTicker = function () {
        $http.get("https://explorer.ebookcoin.com/api/getPriceTicker")
            .then(function (response) {
                $scope.btc_usd = response.data.btc_usd;
                $scope.ebookcoin_btc = response.data.ebookcoin_btc;
                $scope.ebookcoin_usd = response.data.ebookcoin_usd;
            });
    };
    ...
    $scope.getVersion = function () {
        $http.get("/api/peers/version").then(function (response) {
            if (response.data.success) {
                ...
                $http.get("https://login.ebookcoin.com/api/peers/version").then(function (response) {
    ...
    $scope.convertToUSD = function (ebookcoin) {
        return (ebookcoin / 100000000) * $scope.ebookcoin_usd;
    };
